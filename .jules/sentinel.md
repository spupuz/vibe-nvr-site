...
## 2026-08-24 - [Weak Random Number Generation for IDs]
**Vulnerability:** Insecure random identifier generation using `Math.random()`. This function is not cryptographically secure and predictable, which could be abused if IDs are relied on for uniqueness or non-guessability (even anonymous tracking).
**Learning:** Found in `src/scripts.html` during telemetry tracking visitor ID assignment.
**Prevention:** Use `window.crypto.randomUUID()` (or `crypto.getRandomValues()`) for random ID generation when possible to ensure cryptographically secure randomness, maintaining the insecure method only as a fallback for unsupported environments.
