...
## 2024-05-15 - [Improve predictable ID generation]
**Vulnerability:** Usage of `Math.random()` to generate the telemetry visitor ID creates predictable identifiers.
**Learning:** `Math.random()` does not produce cryptographically secure random numbers, making generated IDs guessable. In scenarios requiring uniqueness or some level of anonymity, predictable IDs could be linked or abused if intercepted or inferred.
**Prevention:** Always use cryptographically secure random number generators (CSPRNG) like `window.crypto.randomUUID()` in the browser or `crypto.randomBytes()` in Node.js when generating persistent identifiers or tokens. Implement a fallback for older environments if necessary, but default to the secure method.
