...
## 2026-08-25 - [Predictable ID Generation]
**Vulnerability:** Weak random number generation for security purposes. Generating user IDs with `Math.random()` can be predictable.
**Learning:** Found in `src/scripts.html` during telemetry visitor ID generation, where Math.random() was used.
**Prevention:** Prefer cryptographically secure methods like `window.crypto.randomUUID()` over `Math.random()`, maintaining a fallback for compatibility to avoid predictable ID generation.
