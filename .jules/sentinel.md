...
## 2026-08-23 - [Predictable ID Generation]
**Vulnerability:** The application used Math.random() for generating a persistent visitor ID for telemetry, which relies on a pseudo-random number generator that is not cryptographically secure and could potentially allow ID prediction or collision.
**Learning:** Even for non-sensitive identifiers like a telemetry visitor ID, relying on Math.random() is an anti-pattern. Modern browsers provide window.crypto.randomUUID() which is cryptographically secure, standard, and easy to use.
**Prevention:** Always prefer window.crypto.randomUUID() for generating UUIDs in the browser. When supporting older environments, maintain a fallback mechanism but prioritize the cryptographically secure method.
