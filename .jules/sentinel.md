...
## 2026-08-20 - [Weak PRNG for Telemetry IDs]
**Vulnerability:** The application used  to generate persistent visitor IDs for telemetry in `src/scripts.html`.
**Learning:**  is not a cryptographically secure pseudo-random number generator (CSPRNG), which makes generated IDs predictable. While only for telemetry here, this habit can lead to severe security flaws if used for session tokens or authentication.
**Prevention:** Always use the Web Crypto API, specifically `crypto.randomUUID()` or `crypto.getRandomValues()`, when generating unique identifiers or tokens on the client side.
## 2026-08-20 - [Weak PRNG for Telemetry IDs]
**Vulnerability:** The application used `Math.random()` to generate persistent visitor IDs for telemetry in `src/scripts.html`.
**Learning:** `Math.random()` is not a cryptographically secure pseudo-random number generator (CSPRNG), which makes generated IDs predictable. While only for telemetry here, this habit can lead to severe security flaws if used for session tokens or authentication.
**Prevention:** Always use the Web Crypto API, specifically `crypto.randomUUID()` or `crypto.getRandomValues()`, when generating unique identifiers or tokens on the client side.
