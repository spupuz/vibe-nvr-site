...
## 2024-08-21 - FCP Optimization for Dynamic Components
**Learning:** In the vanilla JS architecture where `index.html` loads HTML component fragments dynamically, utilizing `await Promise.all()` to resolve the fetches blocks the rendering of all components (including critical top-of-page LCP components like `header` and `hero`) until the absolute slowest component finishes downloading.
**Action:** When dynamically loading an ordered list of components concurrently, iterate through the fetch Promises sequentially (e.g., `for (const fetchPromise of fetchPromises) { const result = await fetchPromise; ... }`) instead of using `Promise.all()`. This allows components to render into the DOM exactly in order as soon as they are ready, significantly improving First Contentful Paint (FCP) and Time to Interactive.
