...
## 2026-08-31 - Batching DOM Updates with DocumentFragment
**Learning:** In dynamically loaded fragments (like `telemetry.html`), appending elements sequentially to the live DOM directly (e.g., using a series of `appendChild()` calls on an existing DOM node) triggers multiple synchronous layout and reflow operations, blocking the main thread and slowing down the perceived update time.
**Action:** When dynamically generating multiple child elements and injecting them into the DOM, always append them to a temporary `DocumentFragment` first. Append the fragment to the target node as a single operation to drastically minimize reflows and repaints.
