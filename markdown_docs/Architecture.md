...
- **OS-level I/O Buffering**: Deprecated the use of `flush_packets=0` in the PyAV muxer to prevent severe RAM bloat during prolonged recording sessions. Storage optimization now relies on native OS-level disk buffering (e.g., ZFS ZIL/ARC) without forcefully halting FFmpeg packet flushes.
...
- **Database Query Optimization**: Critical API endpoints (such as `get_users`) employ eager loading techniques (`selectinload`) to resolve N+1 query issues during serialization. Background services (such as the health monitor) use lightweight queries (`get_active_cameras_lightweight`) to prevent memory exhaustion (OOM) and excessive database I/O. Furthermore, aggregations for historical statistics (e.g. `get_stats_history`) are performed directly at the database level to prevent application-side memory bloat.
